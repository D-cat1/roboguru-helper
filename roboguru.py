...
def cari(soal: str, mapel: str, kodse_kelas: str):
    ...
    for list_quest in a["data"]["questions"]:
        if len(list_quest["options"]) == 0:
            imgkit.from_string('<p><b>Soal :</b></p>{}<p><b>Jawaban :</b></p>{}'.format(list_quest["contents"], list_quest['contentDefinition']), 'jawaban/{}.jpg'.format(list_quest['serial']), options={"xvfb": "", "format": "jpg"})
            ...
            for all_answer in list_quest["options"]:
                ...
                abjad += 1
            imgkit.from_string('<p><b>Soal :</b></p>{}{}<p><b>Jawaban :</b></p>{}'.format(list_quest["contents"],dumpopt, list_quest['contentDefinition']), 'jawaban/{}.jpg'.format(list_quest['serial']), options={"xvfb": "", "format": "jpg"})
            datajawaban.append('jawaban/{}.jpg'.format(list_quest['serial']))
    return datajawaban
